...
class GCN(Module):
    def __init__(self, args, user_feature, item_feature, rating):
        ...
        self.user_direction_embedding = nn.Embedding(user_feature['direction'].max() + 1, 8)
        ...
        self.item_age_embedding = nn.Embedding(item_feature['age'].max() + 1, 8)
        self.item_literature_embedding = nn.Embedding(item_feature['literature'].max() + 1, 8)
        self.item_educational_embedding = nn.Embedding(item_feature['educational'].max() + 1, 8)
    ...
    def getFeature(self):
        ...
        direction = self.user_direction_embedding(torch.tensor(self.user_feature['direction']).to(self.device))
        location = self.user_location_embedding(torch.tensor(self.user_feature['location']).to(self.device))
        user_emb = torch.cat((user_id, age, gender, direction, location), dim=1)

        # 根据教师特征获取对应的embedding
        item_id = self.item_id_embedding(torch.tensor(self.item_feature['id']).to(self.device))
        item_age = self.item_age_embedding(torch.tensor(self.item_feature['age']).to(self.device))
        literature = self.item_literature_embedding(torch.tensor(self.item_feature['literature']).to(self.device))
        educational = self.item_educational_embedding(torch.tensor(self.item_feature['educational']).to(self.device))
        title = self.item_title_embedding(torch.tensor(self.item_feature['title']).to(self.device))
        item_emb = torch.cat((item_id, item_age, literature, educational, title), dim=1)

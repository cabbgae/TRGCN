...
item_feature = pd.read_csv('./data/teacher.txt', encoding='utf-8', sep='\t')
...
# model.load_state_dict(torch.load(r'D:\cabbage\论文\code\TRGCN\model\bestModeParms-05-02-22-49.pth'))
# model.eval()
# 训练
...
# 加载最佳模型
# model.load_state_dict(torch.load(r'D:\cabbage\论文\code\TRGCN\model\bestModeParms-03-13-13-08.pth'))
# model.eval()

# 推荐Top-K项目
f_w = open("predictions.txt","w",encoding='utf-8')
for user_id in range(0,900):
  print(user_id)
  ...
  f_w.write(str(predictions)+"\n")

true_labels = f1score.gettrue_labels()
predicted_probs = np.array(f1score.getpredicted_probs())
a = f1score.getoptimal_threshold(true_labels,predicted_probs)
# getoptimal_threshold(true_labels,predicted_probs)
f1score.getscore(true_labels,predicted_probs,a)
